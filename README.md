...
- **Podcast-Integration, Sub-Rubriken & On-Demand Audio:** Intelligente Unterscheidung zwischen Live-Streams und Episoden, echtes Pause & Resume mit Zeiterhalt (kein Zurücksetzen auf 0:00), dynamische Scrubber-Timeline mit Fortschrittsanzeige (`Aktuelle Zeit / Gesamtlänge`), 15s Quick-Skip (`-15s` / `+15s`), ausklappbare Episodenübersicht ("Weitere Folgen") in der Genre-Ansicht, interaktive Rubriken-Filterung (Politik, Finanzen, Technik, Musik) und serverseitiger RSS-Feed-Resolver (`podcast.php`) mit cURL-Engine und 15-Minuten Cache.
- **Weltweite Radio- & Podcast-Suche (> 40.000 Sender & Apple Podcasts API):** Intuitives Tab-System im Admin-Panel zur bequemen Umschaltung zwischen Radio- und Podcast-Recherche. Durchsucht das globale Radio-Browser-Verzeichnis (> 40.000 Stationen) und das Apple Podcasts Verzeichnis (iTunes Search API) direkt per Namenseingabe – ohne manuelle Recherche kryptischer Stream- oder Feed-URLs. Übersichtliche Trefferlisten mit Logos/Covern, Genre/Host, Bitrate und Länderkennung erlauben treffsichere Auswahl gleichnamiger Sender. Per 1-Klick Auto-Fill werden alle Metadaten ins Formular übernommen (inkl. klappbarem RSS-Direktimport und dynamischem "X"-/Ausblenden-Reset).
